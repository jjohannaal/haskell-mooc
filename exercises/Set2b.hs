...
myGcd a 0 = abs a  -- If one number is zero, return the absolute value of the other number
myGcd 0 b = abs b  -- If the other number is zero, return the absolute value of it
myGcd a b = myGcd b (a `mod` b)  -- Apply the Euclidean algorithm recursively
...
leftpad str len
    ...
    where spaces = replicate (len - length str) ' ' 
...
-- Usage: smallestDivisor n finds the smallest divisor of n greater than 1
