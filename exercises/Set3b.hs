...
sorted :: [Int] -> Bool
sorted [] = True  -- Base case: empty list is considered sorted
sorted [_] = True -- Base case: single element list is considered sorted
sorted (x:y:xs)
    | x <= y = sorted (y:xs) -- If x is less than or equal to y then will continue checkingrest of the list
    | otherwise = False      -- If x is greater than y, the list is not sorted
...
map2 _ [] _ = [] 
map2 _ _ [] = []  
map2 f (x:xs) (y:ys) = f x y : map2 f xs ys  -- Apply the function to the heads of both lists and continue
